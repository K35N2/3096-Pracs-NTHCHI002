...
int main(void){
  ...
  while (1)
  {
	...
	sprintf(unixTime, "%d \r\n", epochFromTime(time));
	HAL_UART_Transmit(&huart2, unixTime, sizeof(unixTime), 1000);
...
void pause_sec(float x)
{
	...
	//YOUR CODE HERE
	for (int secs = 0; secs < (int)x; secs++)
	{
...
int epochFromTime(TIME time){
	...
	epochDiff = (day * 86400) + ((time.hour - 2) * 3600) + (time.minutes * 60) + time.seconds ;


/**
	int sum;

    sum = sum + (time.year - 22) * 31556926;
	sum = sum + (time.month - 1) * 2629743;
	sum = sum + (time.dayofmonth - 1) * 86400;
	sum = sum + (time.hour - 2) * 3600;
	sum = sum + time.minutes * 60;
	sum = sum + time.seconds;

	return EPOCH_2022 + sum;
*/

	return EPOCH_2022 + epochDiff;
}
